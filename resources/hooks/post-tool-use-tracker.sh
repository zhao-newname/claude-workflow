...
# Extract relevant data using node (since jq might not be available)
eval "$(echo "$tool_info" | node -e "
const data = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log('tool_name=' + (data.tool_name || ''));
console.log('file_path=' + (data.tool_input?.file_path || ''));
console.log('session_id=' + (data.session_id || ''));
console.log('cwd=' + (data.cwd || ''));
")"

# Set project directory (prefer cwd from input, fallback to env var or current dir)
if [[ -n "$cwd" ]]; then
